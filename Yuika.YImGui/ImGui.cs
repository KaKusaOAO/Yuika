...
public static class ImGui
{
...
#if USE_DOCKING
    internal const uint ViewportDefaultId = 0x11111111;
#endif
    ...
    public static void DockContextInitialize(IImGuiContext context) 
    {
        ImGuiContext? ctx = context as ImGuiContext;
        Debug.Assert(ctx != null);

        throw new NotImplementedException();
    }

    public static void DockContextShutdown(IImGuiContext context) 
    {
        ImGuiContext? ctx = context as ImGuiContext;
        Debug.Assert(ctx != null);
    ...
    public static void DockContextClearNodes(IImGuiContext context, uint rootId, bool clearSettingsRefs) 
    {
    ...
    public static unsafe bool Begin(string name, bool* open, ImGuiWindowFlags flags = ImGuiWindowFlags.None) 
    {
        ...
        ImGuiWindow? window = FindWindowByName(name) as ImGuiWindow;
        ...
        if (firstBeginOfTheFrame)
        {
            ...
            window.ClipRect = new System.Drawing.RectangleF(float.MinValue, float.MinValue, float.MaxValue, float.MaxValue);
            throw new NotImplementedException();
    ...
    public static IImGuiWindow? FindWindowByName(string name) 
    {
        uint id = ImHashStr(name);
        return FindWindowById(id);
    }

    public static IImGuiWindow? FindWindowById(uint id) 
    {
        ImGuiContext ctx = EnsureContext();
        return ctx.WindowsById.GetObject<ImGuiWindow>(id);
    }

    private static uint[] _crc32LookupTable = new uint[256]
    {
