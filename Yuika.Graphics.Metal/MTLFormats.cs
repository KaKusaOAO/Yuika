            ...
            bool cube)
        {
            switch (type)
            {
                case TextureType.Texture1D:
                    return arrayLayers > 1 ? MTLTextureType.k1DArray : MTLTextureType.k1D;
                case TextureType.Texture2D:
                    if (cube)
                    {
                        return arrayLayers > 1 ? MTLTextureType.kCubeArray : MTLTextureType.kCube;
                    }
                    else if (multiSampled)
                    {
                        return MTLTextureType.k2DMultisample;
                    }
                    else
                    {
                        return arrayLayers > 1 ? MTLTextureType.k2DArray : MTLTextureType.k2D;
                    }
