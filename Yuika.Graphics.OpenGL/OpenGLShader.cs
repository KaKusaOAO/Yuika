...
internal unsafe class OpenGLShader : Shader, OpenGLDeferredResource
{
    ...
    {
        _shader = (uint) GL.CreateShader(_shaderType);
        CheckLastError();

        byte* textPtr = (byte*)_stagingBlock.Data;
        int length = (int)_stagingBlock.SizeInBytes;

        string text = new string((sbyte*)textPtr);

        GL.ShaderSource(_shader, 1, new []{text}, ref length);
        CheckLastError();
        ...
        int compileStatus;
        GL.GetShader(_shader, ShaderParameter.CompileStatus, &compileStatus);
        CheckLastError();

        if (compileStatus != 1)
        {
            int infoLogLength;
            GL.GetShader(_shader, ShaderParameter.InfoLogLength, &infoLogLength);
            CheckLastError();

            string infoLog; // = stackalloc byte[infoLogLength];
            int returnedInfoLength;
            GL.GetShaderInfoLog(_shader, infoLogLength, &returnedInfoLength, out infoLog);
            CheckLastError();

            infoLog ??= "<null>";

            throw new VeldridException($"Unable to compile shader code for shader [{_name}] of type {_shaderType}: {infoLog}");
        }
