...
internal unsafe class OpenGLSampler : Sampler, OpenGLDeferredResource
{
    ...
    private class InternalSamplerState
    {
        ...
        public void CreateGLResources(SamplerDescription description, bool mipmapped, GraphicsBackend backend)
        {
            ...
            GL.SamplerParameter(_sampler, SamplerParameterName.TextureWrapS, (int)OpenGLFormats.VdToGLTextureWrapMode(description.AddressModeU));
            CheckLastError();
            GL.SamplerParameter(_sampler, SamplerParameterName.TextureWrapT, (int)OpenGLFormats.VdToGLTextureWrapMode(description.AddressModeV));
            CheckLastError();
            GL.SamplerParameter(_sampler, SamplerParameterName.TextureWrapR, (int)OpenGLFormats.VdToGLTextureWrapMode(description.AddressModeW));
            CheckLastError();
                ...
                || description.AddressModeW == SamplerAddressMode.Border)
            {
                ...
                GL.SamplerParameter(_sampler, SamplerParameterName.TextureBorderColor, (float*)&borderColor);
                CheckLastError();
            }

            GL.SamplerParameter(_sampler, SamplerParameterName.TextureMinLod, description.MinimumLod);
            CheckLastError();
            GL.SamplerParameter(_sampler, SamplerParameterName.TextureMaxLod, description.MaximumLod);
            CheckLastError();
            if (backend == GraphicsBackend.OpenGL && description.LodBias != 0)
            {
                GL.SamplerParameter(_sampler, SamplerParameterName.TextureLodBias, description.LodBias);
                CheckLastError();
            }

            if (description.Filter == SamplerFilter.Anisotropic)
            {
                GL.SamplerParameter(_sampler, SamplerParameterName.TextureMaxAnisotropyExt, description.MaximumAnisotropy);
                CheckLastError();
                GL.SamplerParameter(_sampler, SamplerParameterName.TextureMinFilter, mipmapped ? (int)TextureMinFilter.LinearMipmapLinear : (int)TextureMinFilter.Linear);
                CheckLastError();
            ...
            else
            {
                ...
                GL.SamplerParameter(_sampler, SamplerParameterName.TextureMinFilter, (int)min);
                CheckLastError();
                GL.SamplerParameter(_sampler, SamplerParameterName.TextureMagFilter, (int)mag);
                CheckLastError();
            }

            if (description.ComparisonKind != null)
            {
                GL.SamplerParameter(_sampler, SamplerParameterName.TextureCompareMode, (int)TextureCompareMode.CompareRefToTexture);
                CheckLastError();
                GL.SamplerParameter(_sampler, SamplerParameterName.TextureCompareFunc, (int)OpenGLFormats.VdToGLDepthFunction(description.ComparisonKind.Value));
                CheckLastError();
            }
        }
