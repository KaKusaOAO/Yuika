...
internal unsafe class OpenGLCommandExecutor
{
    ...
    public void Draw(uint vertexCount, uint instanceCount, uint vertexStart, uint instanceStart)
    {
        ...
        {
            GL.DrawArrays(_primitiveType, (int)vertexStart, (int)vertexCount);
            CheckLastError();
        }
        else
        {
            if (instanceStart == 0)
            {
                GL.DrawArraysInstanced(_primitiveType, (int)vertexStart, (int)vertexCount, (int)instanceCount);
                CheckLastError();
            }
            else
            {
                GL.DrawArraysInstancedBaseInstance(_primitiveType, (int)vertexStart, (int)vertexCount, (int)instanceCount, instanceStart);
                CheckLastError();
            }
        }
    }
    ...
    public void DrawIndexed(uint indexCount, uint instanceCount, uint indexStart, int vertexOffset, uint instanceStart)
    {
        ...
        if (instanceCount == 1 && instanceStart == 0)
        {
            if (vertexOffset == 0)
            {
                GL.DrawElements(_primitiveType, (int) indexCount, _drawElementsType, (IntPtr)indices);
                CheckLastError();
            }
            else
            {
                GL.DrawElementsBaseVertex(_primitiveType, (int) indexCount, _drawElementsType, (IntPtr)indices, vertexOffset);
                CheckLastError();
            }
        }
        else
        {
            ...
            {
                GL.DrawElementsInstanced(_primitiveType, (int) indexCount, _drawElementsType, (int) indices, (int) instanceCount);
                CheckLastError();
            }
            else
            {
    ...
    private void ActivateGraphicsPipeline()
    {
        ...
        if (_features.IndependentBlend)
        {
            for (uint i = 0; i < blendState.AttachmentStates.Length; i++)
            {
                ...
                ColorWriteMask colorMask = attachment.ColorWriteMask.GetValueOrDefault(); //.GetOrDefault();
        ...
        else if (blendState.AttachmentStates.Length > 0)
        {
            ...
            ColorWriteMask colorMask = attachment.ColorWriteMask.GetValueOrDefault();
    ...
    public void PushDebugGroup(string name)
    {
        ...
        {
            int byteCount = Encoding.UTF8.GetByteCount(name);
            GL.Ext.PushGroupMarker(byteCount, name); // utf8Ptr);
            CheckLastError();
        }
    }
    ...
    public void InsertDebugMarker(string name)
    {
        ...
        {
            int byteCount = Encoding.UTF8.GetByteCount(name);
            GL.Ext.InsertEventMarker(byteCount, name);
            CheckLastError();
        }
    }
            ...
            switch (kind)
            {
                ...
                case ResourceKind.TextureReadOnly:
                    ...
                    if (pipeline.GetTextureBindingInfo(slot, element, out OpenGLTextureBindingSlotInfo textureBindingInfo))
                    {
                        ...
                        GL.Uniform1(textureBindingInfo.UniformLocation, textureBindingInfo.RelativeIndex);
                        CheckLastError();
                    }
                    break;
                case ResourceKind.TextureReadWrite:
                    ...
                    if (pipeline.GetTextureBindingInfo(slot, element, out OpenGLTextureBindingSlotInfo imageBindingInfo))
                    {
                        ...
                        if (_backend == GraphicsBackend.OpenGL)
                        {
                            ...
                            CheckLastError();
                            GL.Uniform1(imageBindingInfo.UniformLocation, imageBindingInfo.RelativeIndex);
                            CheckLastError();
                        }
                        else
                        {
        ...
        if (texTarget == TextureTarget.Texture1D)
        {
            if (isCompressed)
            {
                GL.CompressedTexSubImage1D(
                    ...
                    (int)x,
                    (int)width,
                    (GLPixelFormat) glTex.GLInternalFormat,
                    (int)rowPitch,
                    dataPtr);
                CheckLastError();
            }
        ...
        uint layerCount)
    {
        ...
        if (_extensions.CopyImage && depth == 1)
        {
            ...
            height = Math.Min(height, mipHeight);
            GL.CopyImageSubData(
                srcGLTexture.Texture, (ImageTarget)srcGLTexture.TextureTarget, (int)srcMipLevel, (int)srcX, (int)srcY, (int)srcZOrLayer,
                dstGLTexture.Texture, (ImageTarget)dstGLTexture.TextureTarget, (int)dstMipLevel, (int)dstX, (int)dstY, (int)dstZOrLayer,
                (int)width, (int)height, (int)depthOrLayerCount);
            CheckLastError();
        }
        else
        {
        ...
        uint width, uint height, uint depth)
    {
        ...
        if (isCompressed)
        {
            ...
            else
            {
                ...
                GL.GetCompressedTexImage(srcTarget, (int)srcMipLevel, (IntPtr)block.Data);
                CheckLastError();
            }
        ...
        else // !isCompressed
        {
            if (_extensions.ARB_DirectStateAccess)
            {
                GL.GetTextureSubImage(
                    ...
                    srcGLTexture.GLPixelFormat, srcGLTexture.GLPixelType, (int) block.SizeInBytes, (IntPtr)block.Data);
                CheckLastError();
            }
            else
            {
